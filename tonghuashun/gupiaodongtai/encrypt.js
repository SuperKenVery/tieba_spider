var thspubkey = {
    "v": "default_4",
    "p": "10001",
    "m": ""
};
...
function bnpFromString(b, a) {
    ...
    for (var d = b.length, e = !1, f = 0; 0 <= --d;) {
        ...
        0 > g ? "-" == b.charAt(d) && (e = !0) : (e = !1, 0 == f ? this[this.t++] = g : f + c > this.DB ? (this[this.t - 1] |= (g & (1 << this.DB - f) - 1) << f, this[this.t++] = g >> this.DB - f) : this[this.t - 1] |= g << f, f += c, f >= this.DB && (f -= this.DB))
    }
    if (8 == c && 0 != (b[0] & 128)) this.s = -1, 0 < f && (this[this.t - 1] |= (1 <<
        this.DB - f) - 1 << f);
    this.clamp();
...
function bnToString(b) {
    ...
    if (0 < f--) {
        if (g < this.DB && 0 < (c = this[f] >> g)) d = !0, e = int2char(c);
        for (; 0 <= f;) g < b ? (c = (this[f] & (1 << g) - 1) << b - g, c |= this[--f] >> (g += this.DB - b)) : (c = this[f] >> (g -= b) & a, 0 >= g && (g += this.DB, --f)), 0 < c && (d = !0), d && (e += int2char(c))
    }
    return d ? e : "0"
}
...
function bnpLShiftTo(b, a) {
    var c = b % this.DB,
        ...
        g = this.s << c & this.DM,
        h;
    for (h = this.t - 1; 0 <= h; --h) a[h + f + 1] = this[h] >> d | g, g = (this[h] & e) << c;
...
function bnpRShiftTo(b, a) {
    ...
    if (c >= this.t) a.t = 0;
        ...
        var d = b % this.DB,
            ...
            f = (1 << d) - 1;
        a[0] = this[c] >> d;
        for (var g = c + 1; g < this.t; ++g) a[g - c - 1] |= (this[g] & f) << e, a[g - c] = this[g] >> d;
...
function bnpSubTo(b, a) {
    for (var c = 0, d = 0, e = Math.min(b.t, this.t); c < e;) d += this[c] - b[c], a[c++] = d & this.DM, d >>= this.DB;
    if (b.t < this.t) {
        for (d -= b.s; c < this.t;) d += this[c], a[c++] = d & this.DM, d >>= this.DB;
        d += this.s
    } else {
        for (d += this.s; c < b.t;) d -= b[c], a[c++] = d & this.DM, d >>= this.DB;
        d -= b.s
    }
...
function bnpSquareTo(b) {
    ...
    for (c = 0; c < a.t - 1; ++c) {
        ...
        if ((b[c + a.t] += a.am(c + 1, 2 * a[c], b, 2 * c + 1, d, a.t - c - 1)) >= a.DV) b[c + a.t] -= a.DV, b[c + a.t + 1] = 1
    }
...
function bnpDivRemTo(b, a, c) {
    ...
    if (!(0 >= d.t)) {
        var e = this.abs();
        if (e.t < d.t) null != a && a.fromInt(0), null != c && this.copyTo(c);
        else {
            null == c && (c = nbi());
            var f = nbi(),
                g = this.s,
            ...
            if (0 != e) {
                var i = e * (1 << this.F1) + (1 < d ? f[d - 2] >> this.F2 : 0),
                    ...
                    j = null == a ? nbi() : a;
                f.dlShiftTo(m, j);
                0 <= c.compareTo(j) && (c[c.t++] = 1, c.subTo(j, c));
                BigInteger.ONE.dlShiftTo(d,
                    j);
                for (j.subTo(f, f); f.t < d;) f[f.t++] = 0;
                for (; 0 <= --m;) {
                ...
                null != a && (c.drShiftTo(d, a), g != b && BigInteger.ZERO.subTo(a, a));
                c.t = d;
                c.clamp();
                0 < h && c.rShiftTo(h, c);
...
function montReduce(b) {
    ...
    for (var a = 0; a < this.m.t; ++a) {
        var c = b[a] & 32767,
            ...
            c = a + this.m.t;
        for (b[c] += this.m.am(0, d, b, a, 0, this.m.t); b[c] >= b.DV;) b[c] -= b.DV, b[++c]++
    }
    b.clamp();
...
function bnpExp(b, a) {
    ...
    var c = nbi(),
        ...
        f = nbits(b) - 1;
    for (e.copyTo(c); 0 <= --f;)
        if (a.sqrTo(c, d), 0 < (b & 1 << f)) a.mulTo(d, e, c);
        else var g = c,
            c = d,
            d = g;
    return a.revert(c)
...
function linebrk(b, a) {
    for (var c = "", d = 0; d + a < b.length;) c += b.substring(d, d + a) + "\n", d += a;
...
function pkcs1pad2(b, a) {
    if (a < b.length + 11) return alert("Message too long for RSA"), null;
    for (var c = [], d = b.length - 1; 0 <= d && 0 < a;) {
        var e = b.charCodeAt(d--);
        128 > e ? c[--a] = e : 127 < e && 2048 > e ? (c[--a] = e & 63 | 128, c[--a] = e >> 6 | 192) : (c[--a] = e & 63 | 128, c[--a] = e >> 6 & 63 | 128, c[--a] = e >> 12 | 224)
    }
    c[--a] = 0;
    d = new SecureRandom;
    for (e = []; 2 < a;) {
...
function RSASetPublic(b, a) {
    null != b && null != a && 0 < b.length && 0 < a.length ? (this.n = parseBigInt(b, 16), this.e = parseInt(a, 16)) : alert("Invalid RSA public key")
}
...
function hex2b64(b) {
    var a, c, d = "";
    for (a = 0; a + 3 <= b.length; a += 3) c = parseInt(b.substring(a, a + 3), 16), d += b64map.charAt(c >> 6) + b64map.charAt(c & 63);
    a + 1 == b.length ? (c = parseInt(b.substring(a, a + 1), 16), d += b64map.charAt(c << 2)) : a + 2 == b.length && (c = parseInt(b.substring(a, a + 2), 16), d += b64map.charAt(c >> 2) + b64map.charAt((c & 3) << 4));
...
function b64tohex(b) {
    ...
    for (c = 0; c < b.length && !(b.charAt(c) == b64padchar); ++c) {
        ...
        0 > v || (0 == d ? (a += int2char(v >> 2), e = v & 3, d = 1) : 1 == d ? (a += int2char(e << 2 | v >> 4), e = v & 15, d = 2) : 2 == d ? (a += int2char(e), a += int2char(v >> 2), e = v & 3, d = 3) : (a += int2char(e << 2 | v >> 4), a += int2char(v & 15), d = 0))
    }
    1 == d && (a += int2char(e << 2));
    return a
}
...
function encryptEncode(b, a, c) {
    if (!a || !c) throw Error("modulus and publicExponent could not be null");
    var d;
    d = new RSAKey;
    d.setPublic(a, c);
    b = d.encrypt(b);
    if (!b) throw Error("encrypt failed");
    return hex2b64(b)
}
thsencrypt.load = function (b, a) {
    ...
    isIE() ? c.onreadystatechange = function (d) {
        if ("loaded" == c.readyState || "complete" == c.readyState) "undefined" !== typeof thspubkey ? (initPubkey(), "function" === typeof b && b(d)) : "function" === typeof a && a(d)
    } : (c.onload = function (a) {
        initPubkey();
